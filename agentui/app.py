...
def receive_external_options():
    ...
    # 从查询参数获取数据
    request_type = request.args.get('type', '1')  # 现在支持1, 2, 0三种类型
    ...
    with options_lock:
        # 检查是否已经有相同选项存在
        if request_type in ["1", "2"] and external_options:
            existing_type = external_options.get('type', '1')
            existing_question = external_options.get('question', '')
            if existing_type == request_type and existing_question == question:
                print(f"已有相同type={request_type}选项存在，跳过设置: {question[:50]}")
                return jsonify({
                    "status": "warning",
                    "message": "相同选项已存在",
                    "type": request_type,
        ...
        print(f"选项数量: {len(options)}, 选项示例: {options[:2] if options else '无'}")
...
def get_options():
    ...
    with options_lock:
        ...
        if external_options:
            ...
            # type=1,2,0都立即清理，确保只返回一次
            if option_type in ["1", "2", "0"]:
                print(f"处理type={option_type}选项，返回后立即清理")
                # 清理全局选项（但先返回数据）
                external_options = None
                # 清理会话选项
